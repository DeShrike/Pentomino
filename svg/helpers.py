...
def lerp_point(value: float, p1: Tuple[float, float], p2: Tuple[float, float]) -> Tuple[float, float]:
	return ( lerp(value, p1[0], p2[0]), lerp(value, p1[1], p2[1]) )

def add_bezier(path:Path, start: Tuple[float, float], stop: Tuple[float, float], c1: Tuple[float, float], c2: Tuple[float, float], steps:int = 10) -> None:
...
def create_hole(x1:float, y1:float, w:float, h:float, id:str, rotation: Rotation = None):
    p = Path(id, True)
    p.color = constants.GREEN
    p.add_node(x1, y1, rotation)
    p.add_node(x1 + w, y1, rotation)
    p.add_node(x1 + w, y1 + h, rotation)
    p.add_node(x1, y1 + h, rotation)
    return p
