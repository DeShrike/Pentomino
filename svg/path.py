...
from .rotation import Rotation, rotate_point
...
class Path():
	...
	def write_to_file(self, fd):
		if len(self.nodes) == 0:
			return
		print(f"    Writing Path: {self.id} - {len(self.nodes)} vertices")
		xml = constants.PATH_XML
		xml = xml.replace("{{ID}}", self.id)
		xml = xml.replace("{{COLOR}}", self.color)
		xml = xml.replace("{{COLORCLASS}}", constants.to_color_class(self.color))
